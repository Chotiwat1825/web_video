...
function runCommand(command, cwd, allowFail = false) {
  ...
  try {
    ...
    if (allowFail) {
      console.warn(`[คำเตือน] คำสั่ง ${command} มีข้อผิดพลาด แต่ระบบจะดำเนินการต่อ:\n${error.message}\n`);
    } else {
...
runCommand('python scrape_heedeng_update.py', 'scrape_heedeng', true);
...
runCommand('python scrape_lovehee_update.py', 'scrape_lovehee', true);
...
runCommand('python scrape_homhee_update.py', 'scrape_homhee', true);
...
runCommand('python scrape_jable_update.py 2', 'scrape_jable', true);
